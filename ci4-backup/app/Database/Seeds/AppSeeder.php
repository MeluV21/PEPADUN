...
class AppSeeder extends Seeder
{
    public function run()
    {
        ...
        // 1. Seed Users (with names matching mockup)
        $users = [
            [
                'username'   => 'admin',
        ...
        // Fetch User IDs
        $userIds = [];
        $usersList = $db->table('users')->get()->getResult();
        foreach ($usersList as $u) {
            $userIds[$u->fullname] = $u->id;
        }

        // 2. Seed Categories (matching mockup exactly)
        $categories = [
            [
                'name'        => 'Profil PPID',
        ...
        // Fetch Category IDs
        $catIds = [];
        $catsList = $db->table('categories')->get()->getResult();
        foreach ($catsList as $c) {
            $catIds[$c->name] = $c->id;
        }

        // 3. Seed Monitoring Data (Mockup dataset: Year 2025, Triwulan I)
        $monitoring = [
            [
        ...
        // Seed some other data for current year 2026 (Triwulan I & II) so it doesn't look empty when they click default links
        $monitoring2026 = [
            [
        ...
        $allMonitoring = array_merge($monitoring, $monitoring2026);
        $db->table('monitoring')->insertBatch($allMonitoring);
    }
}
